...
Throughout this research there will also be many variables that reused. 
In portions of the analysis that copy methods used elsewhere, the assignment 
operator will be used to allow for minimal rewriting in the event that certain 
functions are repurposed for other analysis later on.
...
state and territory since 1963.
...
```
Since the database was created in 1963, the following 10 states have had the 
...
ever occur without being search for on the internet?". According to existing 
...
hits - a numeric integer between 0 and 100 representing search volume as a 
proportion of the maximum search volume for the specified keyword within the 
given time and location bounds. It is worth noting that every Google trends ` will *always*
...
```{r US_climate_change-top4-query, echo=FALSE}
...
```{r FL_hurricane-top4-query, echo=FALSE}
...
```{r CA_fire-top4-query, echo=FALSE}
CA_top4_fires <- c(
"bobcat fire",		
"august fire",		
"creek fire",		
...
CA_top_fire_trends <- gtrends(keyword = CA_top4_fires,
        geo = "US",

...
See README for more thorough description of both `FEMA_Declarations` and 
`M5_Subset` data sets, with content summary, column variable descriptions, 
...
```{r load_packages&data, message=FALSE}
...
num_subsequent_days <- 14

list_of_weather_events <- as.vector(FL_Hurricanes$declaration_title)
list_of_weather_dates <- as.vector(FL_Hurricanes$declaration_day)

finaldata <- as.data.frame(NULL)
for (i in 1:length(list_of_weather_events)) {
  
  start_date <- ymd(list_of_weather_dates[i])

  end_date <- ymd(list_of_weather_dates[i]) + num_subsequent_days
  ...
  all_time_interest <- as.data.frame(weather_event_gtrends[1])
...
n_vec <- as.vector(0:num_subsequent_days) 
coalesce_by_column <- function(df) {
  return(coalesce(df[1], df[2], df[3], df[4], df[5], df[6], df[7], df[8], df[9], df[10], df[11], df[12], df[13], df[14], df[15]))
}
...
a <- finaldata %>% 
      mutate(event_date = gsub( " .*$", "", interest_over_time.time),
             hits_date = interest_over_time.date,
...
finaldata <- finaldata %>% 
  mutate(event_date = gsub( " .*$", "", interest_over_time.time),
         ...
         hits = interest_over_time.hits,
         keyword = interest_over_time.keyword) %>% 
  select(event_date, keyword, hits_date, hits) %>% #event_date, 
  pivot_wider(names_from = hits_date, values_from = hits)

finaldata

for (i in 1:length(list_of_weather_events)) {
  weather_event_gtrends[]$
...
list_of_weather_events <- as.vector(CA_Fires$declaration_title)
...
more than ripples (hits ≤ 5) for the entirety of this past year. Interestingly 

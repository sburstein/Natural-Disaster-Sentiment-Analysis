...
temp_fire_df <- temp_fire_df %>% 
  rename(
    c(
    "indiv_fire_trend...interest_over_time......date..." = "date",
    "indiv_fire_trend...interest_over_time......hits..." = "fire_hits"
    )
  )

temp_cc_df <- temp_cc_df %>% 
  rename(
    indiv_fire_trend...interest_over_time......date... = date,
    indiv_cc_trend...interest_over_time......hits... = cc_hits
  )


#names(temp_fire_df) <- "fire_hits"
#names(temp_cc_df) <- "cc_hits"

date_vector <- as.vector(temp_fire_df$date)
temp_joined <- left_join(temp_fire_df, temp_cc_df, by = date_vector)
#temp_joined

#graph_df <- data.frame(temp)

#graph_trend <- indiv_fire_trend$interest_over_time

ggplot(data = temp_joined) + 
  aes(x = date, y = fire_hits, color = keyword) + 
  geom_line() + 
